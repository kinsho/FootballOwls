	...
	def calculate(self):
		...
		for year, weeks in records.iteritems():
			...
			for team, rate in self.team_rates.iteritems():
				...
				# Ignore any teams for which stats may have not been collected
				if len(rate['early_base_stats']) == 0:
					continue
...
#				self.team_rates[team] = self.team_rates[team]['late_average'] - self.team_rates[team]['early_average']
				...
				results[team] = round(self.team_rates[team], 3)

			formatter.prettify(results, year)
